...
class SendInvitationButtonsTest extends TestCase
{
    ...
    public function test_text_event_sends_rsvp_buttons_in_initial_invite(): void
    {
        ...
        $this->assertSame([
            ...
            "evt:{$event->id}:guest:{$guest->id}:with_children",
    ...
    public function test_image_event_prefers_current_event_asset_url_over_stale_dispatch_url(): void
    {
        $event = Event::query()->create([
            'name' => 'Festa',
            'host_name' => 'Gabriel',
            ...
            'location_name' => 'Buffet Central',
            'location_address' => 'Rua A, 10',
            'invitation_message_template' => 'Olá :guest_name, confirme sua presença.',
            'invitation_asset_type' => 'image',
            'invitation_asset_url' => 'https://nova-url.exemplo/storage/invitation-assets/novo.png',
            'reminder_days_before' => 15,
            'status' => 'draft',
        ]);
        ...
        $dispatch = $event->invitationDispatches()->create([
            ...
            'kind' => InvitationDispatchKind::InitialInvite,
            'message_type' => 'image',
            'outbound_asset_url' => 'https://url-antiga.exemplo/storage/invitation-assets/antigo.png',
            'delivery_status' => InvitationDispatchStatus::Pending,
        ]);
        ...
        $fakeGateway = new class implements WhatsappGateway
        {
            ...
            public function sendImage(string $phone, string $imageUrl, ?string $caption = null, array $options = []): SendResult
            {
                $this->imageCalls[] = compact('phone', 'imageUrl', 'caption', 'options');
        ...
        $this->assertSame(
            'https://math-votes-ground-products.trycloudflare.com/storage/invitation-assets/novo.png',
            $fakeGateway->imageCalls[0]['imageUrl'],
        );
    }

    public function test_older_dispatch_is_not_sent_when_a_newer_one_exists(): void
    {
        $event = Event::query()->create([
            'name' => 'Festa',
            'host_name' => 'Gabriel',
        ...
        $olderDispatch = $event->invitationDispatches()->create([
            'guest_id' => $guest->id,
            'kind' => InvitationDispatchKind::InitialInvite,
            'message_type' => 'text',
            'delivery_status' => InvitationDispatchStatus::Pending,
        ...
        (new SendInvitationJob($olderDispatch->id))->handle(
            $fakeGateway,
            app(\App\Services\InvitationMessageBuilder::class),
        );

        $olderDispatch->refresh();

        $this->assertSame(0, $fakeGateway->buttonCallCount);
        $this->assertSame(InvitationDispatchStatus::Failed, $olderDispatch->delivery_status);
        $this->assertSame('Dispatch substituído por um envio mais recente.', $olderDispatch->failure_reason);
    }
}
